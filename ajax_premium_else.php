<?php
	...
	include_once('process_premium_else.php');
	...
	if(isset($_POST['mode']) && $_POST['mode'] == 'amountChange'){
		...
		foreach($datas as $key => $value){
			if(/*$value['load_amount'] == $selected &&*/ $value['repaid_in'] == $month){
				$diff = abs($value['load_amount'] - $selected);
				if($diff < $min){
					$min = $diff;
